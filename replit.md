...
-   **Shipping & Tracking**: 
    - Quoting: `/api/quotes` returns 10+ valid rates per request with comprehensive Skydropx PRO data including pricing, delivery estimates, extra fees (zona extendida, combustible, manejo adicional), service characteristics (tracking, thermal printing, multipackage, club points), and pickup requirements (sorted by price, cheapest first). QuoteForm includes packaging type selector with 7 options (Caja, Tarima, Sobre, Otros, Saco plástico, Saco papel, Bulto) and envelope weight validation (<1kg visual warning). Weight placeholder simplified to "Peso".
    - Creation: `/api/shipments` creates shipments with automatic wallet deduction. Full Skydropx-style 4-step flow implemented:
      ...
      - **Step 2**: Package information (dimensions, weight with "Peso" placeholder, declared value, Carta Porte, packaging type with 7 options [Caja, Tarima, Sobre, Otros, Saco plástico, Saco papel, Bulto], RFC options, prohibited items warning). Package type validation: envelopes (Sobre) restricted to <1kg with visual warning and backend schema validation.
      - **Step 3**: Carrier selection (sorted by price with "Mejor Precio" badge on cheapest option). Enhanced display showing: service badges (tracking, thermal printing, multipackage, club points), itemized extra fees with amber highlight, pickup vs delivery requirements, and base price vs total when applicable.
